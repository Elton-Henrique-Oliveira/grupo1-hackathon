package projetoL.core.shared.utils
...
class Utils {
    companion object {
        ...
        fun calculateTotalPages(numberOfRegister: Int, pageSize: Int): TotalPages? {

            println(numberOfRegister)
            println(pageSize)
            if (numberOfRegister <= 0 || pageSize <= 0) {
                return null
            }

            val totalPages = numberOfRegister / pageSize
            val remainder = numberOfRegister % pageSize

            val totalPage = TotalPages().apply {
                this.totalPages = totalPages + if (remainder > 0) 1 else 0
